...
† The classes of file I do not type out are the generated ones. There are three, all committed, all carrying a `Code generated ... DO NOT EDIT.` header, and none involving AI at any point:

- The test mocks. The `*_mock.go` files are produced by [mockery](https://github.com/vektra/mockery) from the interface definitions in `.mockery.yml`.
- The database access code. `internal/dao/sqlcgen/*.go` is produced by [sqlc](https://sqlc.dev) from the `.sql` query files in `internal/dao/queries` together with the goose migrations in `internal/db/migrations`, which sqlc uses as the schema to type-check every query against.
- The per-entity query sets and their bindings. `internal/dao/queries/gen/*.sql` and `internal/dao/*_bind_gen.go` are expanded from the templates in `internal/dao/queries/templates` by a small generator in `internal/dao/queries/expand`. That step exists because sqlc requires a literal table name, and the generic DAOs are parameterised by table so that nine comment systems can share one implementation.

All three are deterministic code generation from inputs written by hand, in the same category as a compiler or a protobuf generator. Both generators are pinned in the `go.mod` tool block and run from `./scripts/regen_mocks.sh` and `./scripts/regen_sqlc.sh`; CI reruns them and fails the build if the committed output does not match its inputs.
